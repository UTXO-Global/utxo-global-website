...
export default function BonusReward({ isModalOpen, handleOk, handleCancel }: BonusRewardProps) {
  ...
  const { data, queryConfig, handlePagination, totalData } = usePagination({ limit: 5, inititalData: pointSystemLeaderboardData });
    ...
    <Modal open={isModalOpen} centered footer={false} width={1000} onOk={handleOk} onCancel={handleCancel}>
      <div className="md:pt-5 md:px-4">
        <div className="grid gap-4 text-center">
          <div className="text-3xl font-bold">Exclusive Rewards</div>
          <div>
            <p className="text-orange-100 text-base sm:text-lg font-medium mb-2">Duration: Oct 21 - Oct 28, 2024</p>
            <p className="text-grey-200 text-base sm:text-xl font-medium">
              The top 3 traders who buy $SEAL tokens (applies to buying only, not selling) during the campaign will receive additional $SEAL
              rewards.
            </p>
          </div>
